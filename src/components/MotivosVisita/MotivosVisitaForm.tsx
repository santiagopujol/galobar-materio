...
import { MotivosVisitaService } from '../../services/MotivosVisitaService';
...
const MotivosVisitaForm = ({ data, edit = false }: any) => {
  ...
  const [stateForm, setStateForm] = useState({
    ...
    puntos: data.puntos || "",
    image64: data.image64 || null,
    imageUrl: data.imageUrl || null,
    fechaAbm: data.fechaAbm || "",
  });
      ...
      .then((res) => {
        setting.saveSettings({
          ...
          notificationState: {
            open: true,
            type: "success",
            message: "Motivo de visita guardado con éxito !",
            timeOut: 2000
          },
        })
        router.push('/motivos_visita');
      }).catch((error) => {
        setting.saveSettings({
          ...setting.settings,
          loadingState: false,
      ...
      .then((res) => {
        setting.saveSettings({
          ...
          notificationState: {
            open: true,
            type: "success",
            message: "Motivo de visita eliminado con éxito !",
            timeOut: 2000
          },
        })
        router.push('/motivos_visita');
  ...
  return (
    <Card>
      <CardHeader title={(edit ? 'Editar ' : 'Nuevo ') + 'Motivo de visita'}/>
      <CardContent>
            ...
            {stateForm.id != null &&
              <Grid item xs={6}>
                <Box sx={{ display: 'flex', alignItems: 'left', justifyContent: 'left' }}>
                  <Button color='error' onClick={() => updateStateModalConfirm(setting, true, "eliminar_motivo_visita")}>
                    Eliminar
                  </Button>
                </Box>
              </Grid>
...
export default MotivosVisitaForm
